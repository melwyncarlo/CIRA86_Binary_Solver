PROGRAM CIRA86
    ...
    PRINT  *,  NEW_LINE('1'), ' ENTER 1 FOR PRESSURE-BASED INPUT.',             &
               NEW_LINE('1'), ' ENTER 2 FOR HEIGHT-BASED INPUT.',               &
               NEW_LINE('1')
    ...
    IF (I_ERROR /= 0) THEN
        PRINT *, NEW_LINE('1'), ' ERROR: INVALID INPUT. ENDING PROGRAM.',       &
                 NEW_LINE('1')
        RETURN
    END IF
    ...
    PRINT *, NEW_LINE('1'),                                                     &
             NEW_LINE('1'), ' ENTER 1 FOR TEMPERATURE (IN KELVIN).',            &
             NEW_LINE('1'), ' ENTER 2 FOR ZONAL WIND (IN METERS PER SECOND).',  &
             NEW_LINE('1'),   OUTPUT_TYPE_OPTION_3,                             &
             NEW_LINE('1')
    ...
    PRINT *, NEW_LINE('1'),                                                     &
             NEW_LINE('1'), ' ENTER 1 FOR JANUARY, 2 FOR FEBRUARY, ..., ',      &
                            ' 12 FOR DECEMBER.',                                &
             NEW_LINE('1')
    ...
    IF (I_ERROR /= 0) THEN
        PRINT *, NEW_LINE('1'), ' ERROR: THE FILENAME LABELLED ', ACHAR(39),    &
                                  TRIM(FILENAME), ACHAR(39), NEW_LINE('1'),     &
                                ' EITHER DOES NOT EXIST OR HAS BEEN',           &
                                ' DOCTORED.', NEW_LINE('1'),                    &
                                ' PLEASE DOWNLOAD AGAIN A FRESH COPY OF',       &
                                ' THE MENTIONED FILE.', NEW_LINE('1')
        RETURN
    END IF
    ...
    IF (INPUT_TYPE == 1) THEN
        READ (1) SCALE_HEIGHT, PRESSURE, H, Z
    ELSE IF (INPUT_TYPE == 2) THEN
        READ (1) IH, TW, PP, IEX
    END IF
    ...
    IF (INPUT_TYPE == 1) THEN
        ...
        DO WHILE (I <= I_MAX .AND. PKI <= INPUT_PRESSURE)
            I = I + 1
            PKI = PRESSURE(I)
        END DO
    ...
    WRITE (*, '(A, 50("-"), A, A)')  ' ', NEW_LINE('1'), ' USER INPUTS:'
    WRITE (*, '(A, 50("-"))')  ' '
    ...
    WRITE (*, OUTPUT_FORMAT)     ' INPUT LATITUDE        =  ', INPUT_LATITUDE

    IF (INPUT_TYPE == 1) THEN
        WRITE (*, OUTPUT_FORMAT) ' INPUT PRESSURE VALUE  =  ', INPUT_PRESSURE
    ELSE IF (INPUT_TYPE == 2) THEN
        WRITE (*, OUTPUT_FORMAT) ' INPUT HEIGHT VALUE    =  ', INPUT_HEIGHT
    END IF

    WRITE (*, '(A, A, 50("-"), A, A)')   NEW_LINE('1'), ' ', NEW_LINE('1'),     &
                                       ' OUTPUT RESULTS:'
    WRITE (*, '(A, 50("-"))')  ' '
    ...
    IF (INPUT_TYPE == 1) THEN
        ...
        WRITE (*, OUTPUT_FORMAT)     ' PRESSURE (MB)         =  ', REAL(PKI)
        WRITE (*, OUTPUT_FORMAT)     ' GEOMETRIC HEIGHT      =  ', H(I)
        WRITE (*, OUTPUT_FORMAT)     ' LATITUDE              =  ', REAL(X)
